...
The checked-in release version is **0.0.2**. It is shared by the Rust workspace, the
`clooterminal` launcher, and all four native optional-dependency packages. The current commit has
no release tag.

Release this exact line as `v0.0.2`; do not call it `v1.0.0`. A 1.0.0 release requires a deliberate
...
3. Wait for the `Release artifacts` workflow for that tag to pass. Download its five artifacts into
   ...
   ```text
   clooterminal-darwin-arm64-0.0.2.tgz
   clooterminal-darwin-x64-0.0.2.tgz
   clooterminal-linux-arm64-0.0.2.tgz
   clooterminal-linux-x64-0.0.2.tgz
   clooterminal-0.0.2.tgz
   ```
...
```bash
for package in \
  dist/npm/clooterminal-darwin-arm64-0.0.2.tgz \
  dist/npm/clooterminal-darwin-x64-0.0.2.tgz \
  dist/npm/clooterminal-linux-arm64-0.0.2.tgz \
  dist/npm/clooterminal-linux-x64-0.0.2.tgz
do
  npm --userconfig="$release_npmrc" publish "$package" --access public
done

npm --userconfig="$release_npmrc" publish dist/npm/clooterminal-0.0.2.tgz --access public
```
...
npm view clooterminal-darwin-arm64@0.0.2 version
npm view clooterminal-darwin-x64@0.0.2 version
npm view clooterminal-linux-arm64@0.0.2 version
npm view clooterminal-linux-x64@0.0.2 version

work_dir="$(mktemp -d)"
trap 'rm -rf "$work_dir" "$release_npmrc"' EXIT
npm install --prefix "$work_dir" --ignore-scripts clooterminal@0.0.2
"$work_dir/node_modules/.bin/cloo" --version
```

The final command must report `cloo 0.0.2`. The installed launcher must select the native package
