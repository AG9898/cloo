...
| `XDG_CONFIG_HOME` | No | `~/.config` | Config lookup root. cloo reads `$XDG_CONFIG_HOME/cloo/config.toml`. **Not read yet.** M2-05 landed the *parser* — `cloo-core::config::parse` takes the file's text — but resolving and reading the path is I/O and belongs to the server, which does it at M4-01. |
...
| `CLOO_CONFIG` | No | Derived from `XDG_CONFIG_HOME` | Override the config file path. Intended for tests. **Not read yet** — same reason as `XDG_CONFIG_HOME`. |
...
refusal as of M1-06 — and `XDG_RUNTIME_DIR` and `CLOO_SOCKET` as of M1-01. `XDG_CONFIG_HOME`,
`CLOO_CONFIG`, `NO_COLOR`, and `RUST_LOG` are not.
The table describes the intended surface and is the contract M1 onward implements against. The two
config variables are the last pair still unread: M2-05 landed profile parsing in `cloo-core`, which
takes configuration *text*, so the path resolution they control arrives with the server-side loader
at M4-01.
