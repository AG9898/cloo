...
| `CLOO_SOCKET` | No | Derived from `XDG_RUNTIME_DIR` | Override the socket path. Intended for tests and for running a second daemon alongside a live one. **Read as of M1-01.** It names a socket file, not a directory, and it takes precedence over the session name — the name is ignored entirely when this is set. An empty value counts as unset. Its parent directory is created if missing; a non-socket already sitting at the path is a refusal, never something cloo deletes. As of M2-09 it also moves the adapter control socket, which is always this path plus `.control`. As of M9-09 session discovery treats a non-empty override as its one candidate instead of scanning the runtime directory, but still requires that candidate to be an actual socket answering the versioned read-only inspection handshake. |
