...
| `crates/cloo-client/src/attach.rs` | Attach | A hello completing the attach, `TermCaps` round-tripping over the handshake, a `Tabs` update replacing the cached bar and a resolved command reaching the server, an unresolvable `TERM` surfacing as a capability failure, a refusal surfacing the server's own reason, a future server caught client-side, a non-hello reply and a silent server refused, detach waiting for its acknowledgement, and the live cache placing server grid rows below its tab/header chrome while the matching `ScreenLayout` sends hits to the same pane |
