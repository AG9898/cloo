...
help surface and no longer pane details, `i` still opens details, and an open help surface consumes
navigation, `g`, and Enter locally — Enter closing a reading surface rather than acting. A third
...
M8-04 adds the help surface to that same file as a fourth kind of the one model — it joins the
...
action the user unbound has no row at all, a keymap that binds `?` to a real action takes that row
away while leaving its neighbours, and each row names the `[keys]` action to write when rebinding it
(`client` and `reserved` where there is no such action). A hand-written table of the defaults passes
...
| `crates/cloo-server/src/conn.rs` | Handshake | A matching attach accepted with its `TermCaps` intact field for field, a version mismatch and a non-attach first frame refused with a reason on the wire, a silent peer read as a close, the snapshot batch carrying workspace status first, then tabs before geometry with pane identity and attention before contents, and the session's layout pass carried through rather than recomputed; plus the control handshake: an adapter accepted under its own name, and a report before a hello, a mismatched version, and a name outside the `AdapterId` alphabet each refused |
...
| `crates/cloo-server/src/daemon.rs` | Daemon | Frame-rate cap, fixed IDs, minimum-size arithmetic with degenerate clients counted but excluded from sizing, changed-only workspace-status publication, and a lagged broadcast receiver replacement |
...
| `crates/cloo-client/src/attach.rs` | Attach | A hello completing the attach, `TermCaps` round-tripping over the handshake, `Tabs` and `WorkspaceStatus` updates replacing their caches while a sent resize leaves effective size unchanged until the daemon answers, and a resolved command reaching the server; an unresolvable `TERM` surfacing as a capability failure, a refusal surfacing the server's own reason, a future server caught client-side, a non-hello reply and a silent server refused, detach waiting for its acknowledgement, and the live cache placing server grid rows below its tab/header chrome while the matching `ScreenLayout` sends hits to the same pane; plus client-local visual state — the fresh theme/dimming/status/motion value kept whole, a valid newer revision replacing it atomically, a rejected or duplicate revision preserving it, and two clients sharing one session while resolving different themes; plus copy highlights/status and a dimmed, key-owning overlay layered over that composed frame, with live layout motion restricted to chrome; plus `?` opening the help surface while `i` keeps pane details, an open help surface consuming navigation and Enter locally, and a composed frame drawing the configured prefix's title and its split, detach, and add-pane rows; plus the live launcher — `a` listing exactly the profiles the client was given, a confirmed row handed back as the launch to send, an empty launcher and a dismissal both naming nothing, a launch whose deadline passes becoming a refusal drawn into the composed frame and then clearing, and only an unseen pane of the confirmed profile retiring that notice |
