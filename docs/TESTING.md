...
M9-13 upgrades the card-01 bottom row in that same complete-frame golden. It now expects the
logical session on the accent, a raised and underlined active-tab segment, explicit attention,
right-aligned client detail, and the configured prefix through both the truecolor and 16-color
themes. `chrome.rs` additionally supplies a reference-width fixture with repository and clock data,
the exact optional-to-required yield boundary down to `s>!b`, and pending/first-attach guidance at
every width. `attach.rs` sets deterministic cached clock and repository values beside a daemon
`WorkspaceStatus` and asserts the live composed row contains each source without a numeric-session
placeholder. The status tests are selected with `cargo test -p cloo-client status_bar`.

M9-14 adds six fixtures selected with `cargo test -p cloo-client powerline`. The reference-width
row names `NORMAL`, logical session, marked active tab, repository branch/change count, attached
clients, daemon-projected effective size, and clock; a second fixture proves attention occupies the
same segment only when no repository answer was supplied. Exact narrow strings pin every compaction
rung through the four-cell `Ns>g` floor, and the 16-color fixture asserts identical characters with
indexed semantic roles. A flat-boundary fixture removes only U+E0B0 separator cells and checks the
same fields remain in the same order. Finally, `attach.rs` switches a live client's validated visual
...
- Two visible children writing independently while focus remains fixed: each pane's rows reach the
  attached client, a change isolated to the unfocused child produces damage only for that
  `PaneId`, and changing focus without changing contents does not resend both grids.
- A tab switch fully seeding every pane that becomes visible from the latest server grids while
  inactive-tab output produced no continuous damage traffic.
- A real outer-pseudoterminal capture in which delayed output appears in an unfocused pane without
  a pane clear, periodic full repaint, focus transition, or output-triggered animation. The byte
  assertions reject a full-screen clear and reject repaint spans for unchanged neighbouring rows.
...
- Incremental damage in one unfocused pane repainting only that pane's named row, leaving every
  cached and rendered row of its neighbours untouched and starting no motion phase.
...
M9-12 adds `cloo-client::status` coverage under names containing `status`: an injected fixed clock
pins zero-padded local text and unavailable-time omission; porcelain-v2 fixtures pin branch,
detached-head, and change-count parsing; a real non-repository directory returns no data;
and a deliberately slow child is killed inside the lookup deadline. `attach.rs` changes focus
between two reported working directories and proves the first lookup's late answer is discarded,
so repository data cannot be shown with stale provenance. Production wiring uses
`spawn_blocking`, while the render tick only receives completed optional values and schedules the
one-second refresh.
...
M3-03 adds the always-on status seam; M9-13 replaces its scaffold with the themed segmented minimal
row through the same pure chrome-and-renderer path:

- A wide row carries its logical session, all ordered tabs, per-state actionable tally, optional
  repository/client detail, configured prefix, and clock; the active marker and tally glyphs are
  asserted as text as well as colour.
- Narrow rows follow the fixed yield order rather than dropping fields opportunistically: a
  row loses clock, client/repository detail, inactive tabs, and titles before the four-cell `s>!b`
  form retains one ASCII marker for every required field. An empty queue explicitly says `0!`.
...
| `crates/cloo-server/src/damage.rs` | Damage tracking | First-picture resync for every visible pane, per-pane changed-row-only frames, an unfocused-only change naming only that pane, unchanged neighbours omitted, focus-only changes avoiding grid retransmission, no-op snapshots, exit-frame detection, configuration revisions and workspace status each travelling in their own non-grid frames, and pane identity, attention, and tab selection each resent only when they change rather than on every damaged row |
...
| `crates/cloo-client/src/status.rs` | Client-local status | Clock text from an injected local-time source with unavailable values omitted, porcelain-v2 branch/change parsing including detached heads, non-repositories and failed commands producing no repository value, and a slow child killed at the bounded lookup deadline |
